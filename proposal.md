...
**镜头边界不是输入**：评测器始终用双信号（HSV 直方图 + 降采样像素差分）加 cross-gap 验证的算法从视频本身检测 shot 边界，不依赖外部提供的切分——cross-gap 验证还能区分真切变与生成视频常见的分块生成接缝。我们不使用参考视频，也不依赖逐元素的人工 continuity contract。评测主流程由确定性视觉模块驱动、可自动运行；caption 只负责提供外部要求和镜头语义，模型自由生成出的额外实体与背景则由评测器从视频中在线检测、聚类和比较。
...
真实影视视频提供多镜头结构和连续性压力，但不直接作为 ground truth。发布的是原创 episode 的 **prompt(caption) + 生成视频 + 评测器检测的镜头边界（供审计） + 自动诊断结果**。
...
### 7.7 Stage 7 — 同视角背景一致性
在每个 same-view group 内计算背景 embedding、depth、edge 的组内一致性，并按组内 pair 数加权汇总为 episode 级同视角一致性。
...
### 8.1 按元素类型组织的指标：Coverage × Correctness，不混为单一总分

我们按元素类型把指标组织为三大块，而非一个黑箱总分；人物与物体内部各分 prompt-specified 与 model-emergent 两条 track：

- **Characters**：prompt track 报告 presence / face detection rate、face mean / min / centroid similarity、identity pass rate；emergent track 报告 count / recurrence rate、face mean / min / centroid similarity、identity fragmentation rate。衡量 prompt 要求的人物是否出现且身份稳定，以及模型自发人物是否自洽。
- **Objects**：prompt track 报告 presence、object mean / min / centroid similarity、pass rate；emergent track 报告 count / recurrence rate、object mean / min / centroid similarity、fragmentation rate。小物体相似度融合颜色直方图。
- **Background**：不做实体级拆分，报告 group count / size、intra-group bg / depth / edge similarity、episode_same_view_consistency。
...
### 8.2 分数的分层报告
除总体分数外，按元素类型（人物 / 物体 / 背景）、按 track（prompt-specified / model-emergent）、按镜头结构分别报告一致性分数分布，并报告 fragmentation 等结构性统计。用于揭示模型是易丢道具、易破坏空间关系，还是在特定镜头结构中一致性更低——全部以连续分数与统计量呈现，不设错误判定阈值。

### 8.3 只报分数，不做检测判定
本 benchmark 输出连续分数（embedding 相似度、presence / recurrence rate、组内一致性等），可复现、可审计；它不把生成结果判为"有错 / 无错"——如何据分数排序或设线，留给使用者按需应用。指标中的 `*_pass_rate` 是阈值参数化的统计量（阈值随结果文件一并公布、可复算），刻画相似度分布越过给定水位的比例，不构成判定。缺乏可比证据（背景可见比例过低、无同视角对）的维度直接报为无可比机会（coverage=0），不计入一致性分数。
